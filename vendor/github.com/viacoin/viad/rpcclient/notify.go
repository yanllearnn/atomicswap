...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *viautil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *viautil.Tx, details *btcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount viautil.Amount)
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*viautil.Tx, error) {
	...
	transactions := make([]*viautil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = viautil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*viautil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return viautil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := viautil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance viautil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := viautil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []viautil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []viautil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []viautil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a viad extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []viautil.Address,
...
// NOTE: This is a viad extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []viautil.Address, outPoints []wire.OutPoint) error {
