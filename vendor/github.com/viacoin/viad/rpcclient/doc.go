...
Bitcoin RPC server that uses a viad/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with viad (https://github.com/viacoin/viad),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, viad and
...
viad or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by viad and
...
All notifications provided by viad require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by viad (and
...
Also, it is important to realize that viad intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the viad RPC server directly, only the RPCs which are related to
